...
app.patch("/jokes", (req, res) => {
  ...
  const editedJoke = {
    id: id,
    jokeText: req.boby.text || existingJoke.text,
    jokeType: req.body.type || existingJoke.type,
  };
  jokes.push(editedJoke);
  console.log(jokes.slice(-1));
  res.json(editedJoke);
})
...
const requireKey = (req, res, next) => {
  ...
  if (!providedKey  || providedKey !== `Bearer ${masterKey}`) {
    return res.sendStatus(401).json({error: "Unauthorised"});
  }

  next();
}
app.delete("/all", requireKey, (req, res) => {
  jokes.splice(0);
  res.sendStatus(200);
});
